...
def compute_itc_reid(
    ...
    """
    ITC for Text-based Person ReID
    - weaker than SDM
    ...
    # re-scale (DO NOT use raw CLIP logit_scale)
...
def compute_sdm_hard_aware(
    ...
    # focal-style hard reweight
    i2t_weight = (1.0 - i2t_pred) ** gamma
    t2i_weight = (1.0 - t2i_pred) ** gamma

    i2t_loss = i2t_weight * i2t_pred * (
        F.log_softmax(image_proj_text, dim=1) - torch.log(labels_distribute + epsilon)
    )
    t2i_loss = t2i_weight * t2i_pred * (
...
def compute_id_image_only(image_logits, labels):
    """
    ID loss for Text-based ReID (image only)
    """
    criterion = nn.CrossEntropyLoss(reduction="mean")
...
def compute_memory(image_logits, text_logits, labels):
    """
    Instance loss proposed at http://arxiv.org/abs/1711.05535
    """
    criterion = nn.CrossEntropyLoss(reduction="mean")
