...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,all_layer_outputs=False):
        ...
        if not all_layer_outputs:
            ...
            return cur_output, feature_maps
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image,all_bool):
        return self.visual(image.type(self.dtype),all_layer_outputs=all_bool)

    def encode_text(self, text, return_token_embeddings=False):
        ...
        # Store token embeddings before projection
        token_embeddings = x.clone()  # [batch_size, n_ctx, transformer.width]
        ...
        if return_token_embeddings:
            # Also project token embeddings
            token_embeddings_projected = token_embeddings @ self.text_projection
            return x, token_embeddings_projected
        return x

    def forward(self, image, text, return_token_embeddings=False):
        image_features = self.encode_image(image,False)
        
        if return_token_embeddings:
            text_features, text_token_embeddings = self.encode_text(text, return_token_embeddings=True)
        else:
            text_features = self.encode_text(text)
            text_token_embeddings = None

        _, image_fine = self.encode_image(image, True)
        image_fine_list = []
        top_image_fine_list = []
        ...
        if return_token_embeddings:
            return image_features, text_features, image_fine_list, top_image_fine_list, text_token_embeddings
        return image_features, text_features, image_fine_list, top_image_fine_list
